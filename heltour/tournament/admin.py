...
class SeasonAdmin(_BaseAdmin):
    ...
    def pre_round_report_view(self, request, object_id):
        ...
        context = {
            ...
            if bad_player_status is not None
            else None,
    ...
    def team_manage_players_view(self, request, object_id):
        ...
        expected_ratings = {
            sp.player: sp.expected_rating(league) for sp in season_player_objs
        }
        season_started = Round.objects.filter(
            season=season, publish_pairings=True
        ).exists()
...
class RegistrationAdmin(_BaseAdmin):
    ...
    def get_search_fields(self, request):
        return self.remove_email_if_no_dox(
            request.user, ("lichess_username", "email", "season__name")
        )
    ...
    def valid(self, obj):
        ...
        elif obj.validation_ok is False:
    ...
    def validate(self, request, queryset):
        for reg in queryset:
            signals.do_validate_registration.send(
                sender=RegistrationAdmin, reg_id=reg.pk
            )
