    ...
    def test_slack_link(self, *args):
        ...
        LoginToken.objects.create(
            ...
            expires=datetime.datetime(
                2019, 1, 1, 10, 30, 0, tzinfo=datetime.timezone.utc
            ),
        )
...
class LoginBadTestCase(TestCase):
    ...
    def test_bad_response(self, *args):
        ...
        with Shush():
            response = self.client.get(
                reverse("lichess_auth"), {"code": "abc"}, follow=True
            )
            self.assertTemplateUsed(response, "tournament/login_failed.html")
            response = self.client.get(
                reverse("lichess_auth"), {"state": "abc"}, follow=True
            )
            self.assertTemplateUsed(response, "tournament/login_failed.html")
            response = self.client.get(
                reverse("lichess_auth"), {"code": "abc", "state": "abc"}, follow=True
            )
        self.assertTemplateUsed(response, "tournament/login_failed.html")
