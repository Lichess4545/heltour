...
from heltour.tournament.models import League, LeagueModerator
    ...
    def has_perm(self, user_obj, perm, obj=None):
        if isinstance(obj, League):
            return LeagueModerator.objects.filter(
                league=obj, player__lichess_username__iexact=user_obj.username
            ).exists()
        if obj is None and perm.startswith("tournament.delete_"):
            ...
            return LeagueModerator.objects.filter(
                player__lichess_username__iexact=user_obj.username
            ).exists()
        return False
