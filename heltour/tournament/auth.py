...
class LeagueAuthBackend(object):
    ...
    def has_perm(self, user_obj, perm, obj=None):
        if isinstance(obj, League):
            return LeagueModerator.objects.filter(league=obj, player__user__username__iexact=user_obj.username).exists()
        if obj is None and perm.startswith('tournament.delete_'):
            ...
            return LeagueModerator.objects.filter(player__user__username__iexact=user_obj.username).exists()
        return False
