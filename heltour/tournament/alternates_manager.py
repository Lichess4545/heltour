...
def current_searches(round_):
    active_searches = (
        AlternateSearch.objects.filter(round=round_, is_active=True)
...
def reset_alternate_search(season, round_, setting):
    ...
    for alt in Alternate.objects.filter(season_player__season=season):
        ...
        if (
            ...
            and alt.last_contact_date is not None
            and timezone.now() - alt.last_contact_date > setting.unresponsive_interval
        ):
    ...
    if last_round is not None:
        for search in AlternateSearch.objects.filter(
            round=last_round, status__in=("started", "all_contacted")
        ):
            if search.still_needs_alternate():
                with reversion.create_revision():
                    ...
                    search.save()
                signals.alternate_search_failed.send(
                    sender=do_alternate_search,
                    season=season,
                    team=search.team,
    ...
    if (
        some_bucket is None
        or some_bucket.date_modified < timezone.now() - _min_bucket_update_interval
    ):
...
def do_alternate_search(season, round_, board_number, setting):
    ...
    availability_modified_dates = {
        pa.player: pa.date_modified for pa in player_availabilities
    }
    round_pairings = (
    ...
    players_in_round = {p.white for p in round_pairings} | {
        p.black for p in round_pairings
    }
    if round_.publish_pairings:
        ...
        players_on_board = {p.white for p in board_pairings} | {
            p.black for p in board_pairings
        }
        ...
        team_members = (
            TeamMember.objects.filter(team__season=season, board_number=board_number)
            .select_related("team", "player")
            .nocache()
        )
        assigned_alts_qs = AlternateAssignment.objects.filter(
            round=round_, board_number=board_number
        )
        assigned_alts_lookup = {aa.team: aa.player for aa in assigned_alts_qs}
        players_on_board = {
            assigned_alts_lookup.get(tm.team, tm.player) for tm in team_members
        }
    ...
    players_that_need_replacements = sorted(
        players_on_board & unavailable_players,
        key=lambda p: availability_modified_dates[p],
    )
    alternates_contacted = Alternate.objects.filter(
        season_player__season=season, board_number=board_number, status="contacted"
    )
    last_declined_alternate = (
        Alternate.objects.filter(
            season_player__season=season, board_number=board_number, status="declined"
        )
    ...
    alternates_not_contacted = sorted(
        Alternate.objects.filter(
            season_player__season=season, board_number=board_number, status="waiting"
        )
    ...
    if len(players_that_need_replacements) == 0:
        ...
        for alt in alternates_contacted:
            if (
                alt.last_contact_date is not None
                and timezone.now() - alt.last_contact_date
                > setting.unresponsive_interval
            ):
                alt.status = "unresponsive"
            else:
                alt.status = "waiting"
            with reversion.create_revision():
                ...
                alt.save()
            signals.alternate_spots_filled.send(
                sender=do_alternate_search,
                alternate=alt,
    ...
    for p in players_that_need_replacements:
        search, created = AlternateSearch.objects.get_or_create(
            round=round_, team=teams_by_player[p], board_number=board_number
        )
        ...
        if created or search.status == "completed":
            ...
            signals.alternate_search_started.send(
                sender=do_alternate_search,
                season=season,
        ...
        if search.last_alternate_contact_date is None:
            ...
            last_declined_alternate is not None
            and search.last_alternate_contact_date
            == last_declined_alternate.last_contact_date
        ):
            ...
            time_since_last_contact = (
                timezone.now() - search.last_alternate_contact_date
            )
            contact_interval = (
                setting.contact_interval
                if round_.publish_pairings
                else setting.contact_interval_before_round_start
            )
        ...
        if do_contact:
            try:
                ...
                while True:
                    ...
                    if (
                        alt_to_contact.season_player.player not in unavailable_players
                        and alt_to_contact.season_player.player not in players_in_round
                        and alt_to_contact.season_player.games_missed < 2
                    ):
                        break
                ...
                accept_url = reverse(
                    "by_league:by_season:alternate_accept",
                    args=[league_tag, season_tag, round_.number],
                )
                decline_url = reverse(
                    "by_league:by_season:alternate_decline",
                    args=[league_tag, season_tag, round_.number],
                )
                signals.alternate_needed.send(
                    sender=do_alternate_search,
                    alternate=alt_to_contact,
                    response_time=setting.unresponsive_interval,
                    round_=round_,
                ...
                if search.status != "all_contacted":
                    signals.alternate_search_all_contacted.send(
                        sender=do_alternate_search,
                        season=season,
                        team=teams_by_player[p],
                        board_number=board_number,
                        round_=round_,
                        number_contacted=len(alternates_contacted),
                    )
                    time.sleep(1)
                    with reversion.create_revision():
...
def round_pairings_published(round_):
    ...
    for search in active_searches:
        if search.still_needs_alternate():
            signals.alternate_search_reminder.send(
                sender=round_pairings_published,
...
def alternate_accepted(alternate):
    ...
    if (
        (
            TeamPlayerPairing.objects.filter(
                team_pairing__round=round_, white=alternate.season_player.player
            )
            | TeamPlayerPairing.objects.filter(
                team_pairing__round=round_, black=alternate.season_player.player
            )
        )
        .nocache()
        .exists()
    ...
    active_searches = (
        AlternateSearch.objects.filter(
            round=round_, board_number=alternate.board_number, is_active=True
        )
        .order_by("date_created")
        .select_related("team")
        .nocache()
    ...
    for search in active_searches:
        if search.still_needs_alternate():
            with reversion.create_revision():
                ...
                assignment, _ = AlternateAssignment.objects.update_or_create(
                    round=round_,
                    team=search.team,
                    board_number=search.board_number,
                    defaults={
            ...
            with reversion.create_revision():
                ...
                search.save()
            signals.alternate_assigned.send(
                sender=alternate_accepted, season=season, alt_assignment=assignment
            )
            time.sleep(1)
            return True
    return False
