...
def import_team_season(
    league, url, name, tag, rosters_only=False, exclude_live_pairings=False
):
    doc = _open_doc(url)
    ...
    with transaction.atomic():
        ...
        try:
            ...
            while True:
                ...
                team = Team.objects.create(
                    season=season, number=len(teams) + 1, name=team_name
                )
            ...
            alternates_start_row = [
                row[0] == "Alternates" for row in sheet_rosters
            ].index(True)
            for i in range(season.boards):
                ...
                for i in range(len(teams)):
                    ...
                    TeamMember.objects.get_or_create(
                        team=teams[i],
                        board_number=board,
                        defaults={"player": player, "is_captain": is_captain},
                    )
                # Alternates
                ...
                while True:
                    ...
                    Alternate.objects.get_or_create(
                        season_player=season_player, defaults={"board_number": board}
                    )
                    alternates_row += 1
        ...
        if not rosters_only:
            try:
                ...
                sheet_fixed_pairings = _trim_cells(
                    doc.worksheet("Fixed Pairings").get_all_values()
                )
                ...
                for row in sheet_fixed_pairings:
                    ...
                    white_team_name = (
                        row[1]
                        if board_number == board_count or board_number % 2 == 0
                        else row[4]
                    )
                    white_team = Team.objects.get(
                        season=season, name__iexact=white_team_name
                    )
                    ...
                    black_team = Team.objects.get(
                        season=season, name__iexact=black_team_name
                    )
                    ...
                    team_pairing, created = TeamPairing.objects.get_or_create(
                        ...
                        defaults={"pairing_order": pairing_order},
                    )
                    if created:
                        pairing_order += 1
                        board_number = 1
                    ...
                    SeasonPlayer.objects.get_or_create(
                        season=season, player=white_player
                    )
                    ...
                    SeasonPlayer.objects.get_or_create(
                        season=season, player=black_player
                    )
                    ...
                    colors_reversed = (
                        sheet_fixed_pairings[0][7] == "reversed" and len(row[7]) > 0
                    )
            ...
            sheet_past_rounds = _trim_cells(
                doc.worksheet("Past Rounds").get_all_values()
            )
            ...
            for round_ in rounds:
                ...
                try:
                    round_start_row = [
                        row[0] == "Round %d" % round_number for row in sheet_past_rounds
                    ].index(True)
                except ValueError:
                ...
                result_col = _read_team_pairings(
                    sheet_past_rounds,
                    header_row,
                    season,
                    teams,
            ...
            _update_pairing_game_links(
                doc.worksheet("Past Rounds"), pairings, pairing_rows, result_col
            )
            ...
            if not exclude_live_pairings:
                ...
                _update_pairing_game_links(
                    doc.worksheet(current_round_name),
                    pairings,
                    pairing_rows,
                    result_col,
                )
...
def _read_team_pairings(
    sheet, header_row, season, teams, round_, pairings, pairing_rows
):
        ...
        for k in range(season.boards):
            ...
            if "/" in date:
                ...
                if round_.end_date is None or game_end_estimate > round_.end_date:
                    ...
                    round_.save()
            pairing = TeamPlayerPairing.objects.create(
                team_pairing=team_pairing,
                board_number=k + 1,
                white=white_player,
                black=black_player,
...
def _update_pairing_game_links(worksheet, pairings, pairing_rows, game_link_col):
    if len(pairings) > 0:
        ...
        range_game_links = worksheet.range(
            "%s%d:%s%d"
            % (game_link_col_letter, 1, game_link_col_letter, pairing_rows[-1] + 1)
        )
...
def import_lonewolf_season(
    ...
    with transaction.atomic():
        ...
        try:
            ...
            sheet_standings = _trim_cells(
                doc.worksheet("STANDINGS FINAL").get_all_values()
            )
            season_complete = True
        try:
            sheet_changes = _trim_cells(
                doc.worksheet("PLAYER CHANGES").get_all_values()
            )
        except WorksheetNotFound:
            sheet_changes = None
        ...
        # Create the season
        season = Season.objects.create(
            league=league, name=name, tag=tag, rounds=round_count, is_active=False
        )
        ...
        while row < len(sheet_standings):
            ...
            player, _ = Player.objects.update_or_create(
                lichess_username__iexact=name,
                defaults={"lichess_username": name, "rating": rating},
            )
            season_player, _ = SeasonPlayer.objects.get_or_create(
                season=season, player=player, defaults={"seed_rating": rating}
            )
            ...
            LonePlayerScore.objects.create(
                season_player=season_player,
                points=points,
                late_join_points=ljp,
                tiebreak1=tb1,
                tiebreak2=tb2,
                tiebreak3=tb3,
                tiebreak4=tb4,
        ...
        if sheet_changes is not None:
            ...
            for row in range(1, len(sheet_changes)):
                ...
                rating = (
                    int(sheet_changes[row][rating_col])
                    if len(sheet_changes[row][rating_col]) > 0
                    else None
                )
                ...
                if action == "register":
                    ...
                    PlayerWithdrawal.objects.create(
                        round=season.round_set.get(number=round_number), player=player
                    )
                elif action == "half-point-bye":
                    PlayerBye.objects.create(
                        round=season.round_set.get(number=round_number),
                        player=player,
                        type="half-point-bye",
                    )

        # TODO: Infer missing round changes from the standings page
        round_cols = [  # noqa: F841 - delete if not being used
            (n, sheet_standings[0].index("Rd %d" % n))
        ...
        for round_ in season.round_set.all():
            try:
                ...
                try:
                    worksheet = doc.worksheet(
                        "ROUND %s PAIRINGS" % numbers[round_.number]
                    )
                except WorksheetNotFound:
                    continue
            ...
            for row in range(1, len(sheet)):
                ...
                white_player, _ = Player.objects.get_or_create(
                    lichess_username__iexact=white_player_name,
                    defaults={
                        "lichess_username": white_player_name,
                        "rating": white_player_rating,
                    },
                )
                SeasonPlayer.objects.get_or_create(
                    season=season,
                ...
                if sheet[row][black_col] == "BYE":
                    PlayerBye.objects.update_or_create(
                        round=round_,
                        player=white_player,
                        type="half-point-bye",
                ...
                black_player, _ = Player.objects.get_or_create(
                    lichess_username__iexact=black_player_name,
                    defaults={
                        "lichess_username": black_player_name,
                        "rating": black_player_rating,
                    },
                )
                SeasonPlayer.objects.get_or_create(
                    season=season,
