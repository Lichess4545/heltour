...
class RoundTransitionWorkflow:
    ...
    def round_to_close(self):
        return (
            self.season.round_set.filter(publish_pairings=True, is_completed=False)
            .order_by("number")
            .first()
        )
    ...
    def round_to_open(self):
        return (
            self.season.round_set.filter(publish_pairings=False, is_completed=False)
            .order_by("number")
            .first()
        )
    ...
    def season_to_close(self):
        ...
        return (
            self.season
            if not self.season.is_completed
            and round_to_open is None
            and (round_to_close is None or round_to_close.number == self.season.rounds)
            else None
        )
    ...
    def run(
        ...
        with transaction.atomic():
            if complete_round and round_to_close is not None:
                ...
                msg_list.append(
                    (
                        "Round %d set as completed." % round_to_close.number,
                        messages.INFO,
                    )
                )
            if (
                complete_season
                and season_to_close is not None
                ...
                msg_list.append(
                    ("%s set as completed." % season_to_close.name, messages.INFO)
                )
            if (
                update_board_order
                and round_to_open is not None
                and self.season.league.competitor_type == "team"
            ):
                try:
            ...
            if generate_pairings and round_to_open is not None:
                if background:
                    signals.do_generate_pairings.send(
                        sender=self.__class__, round_id=round_to_open.pk
                    )
                    msg_list.append(
                        ("Generating pairings in background.", messages.INFO)
                    )
                else:
                    try:
                        ...
                        msg_list.append(
                            ("Unpublished pairings already exist.", messages.WARNING)
                        )
    ...
    def warnings(self):
        ...
        if (
            round_to_close is not None
            and round_to_close.end_date is not None
            and round_to_close.end_date > timezone.now() + timedelta(hours=1)
        ):
            time_from_now = self._time_from_now(
                round_to_close.end_date - timezone.now()
            )
            msg_list.append(
                (
                    "The round %d end date is %s from now."
                    % (round_to_close.number, time_from_now),
                    messages.WARNING,
                )
            )
        elif (
            round_to_open is not None
            and round_to_open.start_date is not None
            and round_to_open.start_date > timezone.now() + timedelta(hours=1)
        ):
            time_from_now = self._time_from_now(
                round_to_open.start_date - timezone.now()
            )
            msg_list.append(
                (
                    "The round %d start date is %s from now."
                    % (round_to_open.number, time_from_now),
                    messages.WARNING,
                )
            )
        ...
        if round_to_close is not None:
            incomplete_pairings = (
                PlayerPairing.objects.filter(
                    result="", teamplayerpairing__team_pairing__round=round_to_close
                ).nocache()
                | PlayerPairing.objects.filter(
                    result="", loneplayerpairing__round=round_to_close
                ).nocache()
            )
            if len(incomplete_pairings) > 0:
                msg_list.append(
                    (
                        "Round %d has %d pairing(s) without a result."
                        % (round_to_close.number, len(incomplete_pairings)),
                        messages.WARNING,
                    )
                )
...
class UpdateBoardOrderWorkflow:
    ...
    def run(self, alternates_only):
        ...
        if (
            alternates_only
            or not self.season.alternates_manager_enabled()
            or self.season.round_set.filter(publish_pairings=True).count() == 0
        ):
            members_by_board = [
                TeamMember.objects.filter(team__season=self.season, board_number=n + 1)
                for n in range(self.season.boards)
            ]
            ratings_by_board = [
                sorted([float(m.player.rating_for(self.season.league)) for m in m_list])
                for m_list in members_by_board
            ]
            alternates = (
            ...
            self.smooth_alternate_boundaries(
                boundaries, alternates, ratings_by_board, flex
            )
            ...
            with reversion.create_revision():
                ...
                def invariant(board_num):
                    search = AlternateSearch.objects.filter(
                        round=alternate_search_round,
                        team=team,
                        board_number=board_num,
                        is_active=True,
                    ).first()
                    assignment = AlternateAssignment.objects.filter(
                        round=alternate_search_round, team=team, board_number=board_num
                    ).first()
                ...
                while True:
                    ...
                    for i in range(len(occupied_boards) - 1):
                        ...
                        higher_rtg = (
                            higher_bd.player.rating_for(self.season.league) or 0
                        )
                ...
                reversion.set_comment(
                    "Update board order - %s." % ", ".join(change_descriptions)
                )
    ...
    def calc_alternate_boundaries(self, ratings_by_board):
        ...
        left_average_by_board = [
            sum(r_list[1 : int(len(r_list) / 2)]) / (int(len(r_list) / 2) - 1)
            if len(r_list) > 2
            else sum(r_list) / len(r_list)
            if len(r_list) > 0
            else None
            for r_list in ratings_by_board
        ]
        right_average_by_board = [
            sum(r_list[int((len(r_list) + 1) / 2) : -1]) / (int(len(r_list) / 2) - 1)
            if len(r_list) > 2
            else sum(r_list) / len(r_list)
            if len(r_list) > 0
            else None
        ...
        for i in range(self.season.boards + 1):
            ...
            right_i = i
            while (
                right_i < self.season.boards and right_average_by_board[right_i] is None
            ):
                right_i += 1
            right = (
                right_average_by_board[right_i]
                if right_i < self.season.boards
                else None
            )
    ...
    def smooth_alternate_boundaries(
        self, boundaries, alternates, ratings_by_board, flex
    ):
        ...
        if all((len(r_list) >= 4 for r_list in ratings_by_board)):
            ...
            for i in range(self.season.boards - 1):
                ...
                higher_board_min = (
                    ratings_by_board[i][0] + ratings_by_board[i][1]
                ) / 2 - flex
                lower_board_max = (
                    ratings_by_board[i + 1][-1] + ratings_by_board[i + 1][-2]
                ) / 2 + flex
            ...
            for _ in range(iter_count):
                ...
                for i in range(self.season.boards - 1):
                    ...
                    if (
                        bucket_counts[i] > bucket_counts[i + 1] + 1
                        or bucket_counts[i] == bucket_counts[i + 1] + 1
                        and actual_on_left > expected_on_left + 1
                    ):
        ...
        with reversion.create_revision():
            ...
            for board_num in range(1, self.season.boards + 1):
                ...
                if min_rating is None and max_rating is None:
                    AlternateBucket.objects.filter(
                        season=self.season, board_number=board_num
                    ).delete()
                else:
                    AlternateBucket.objects.update_or_create(
                        season=self.season,
                        board_number=board_num,
                        defaults={"max_rating": max_rating, "min_rating": min_rating},
                    )
...
class ApproveRegistrationWorkflow:
    def __init__(self, reg, round_number=None):
        ...
        self.player = Player.objects.filter(
            lichess_username__iexact=self.reg.lichess_username
        ).first()
    ...
    def default_byes(self):
        ...
        if self.player:
            ...
            for round_number in range(
                self.active_round_count - bye_count + 1, self.active_round_count + 1
            ):
    ...
    def default_section(self):
        ...
        if (
            self.reg.section_preference is not None
            and self.reg.section_preference.is_eligible(player)
        ):
    ...
    def default_ljp(self):
        ...
        if self.default_byes < active_round_count:
            season_players = (
                season.seasonplayer_set.filter(is_active=True)
                .select_related("player", "loneplayerscore")
                .nocache()
            )
            rating = (
                self.reg.classical_rating
                if self.player is None
                else self.player.rating_for(self.league)
            )
            ...
            while diff < 500:
                close_players = [
                    ...
                    if abs(sp.player.rating_for(self.league) - rating) < diff
                ]
                if len(close_players) >= 5:
                    break
                diff += 100
            close_player_scores = sorted(
                [sp.get_loneplayerscore().points for sp in close_players]
            )
            ...
            if len(close_player_scores_adjusted) > 0:
                ...
                if (
                    ...
                    and season.round_set.filter(
                        publish_pairings=True, is_completed=False
                    ).count()
                ):
                    expected_score = (
                        average_score * active_round_count / (active_round_count - 1)
                    )
                else:
    ...
    def approve_reg(
        ...
        if self.is_late:
            # Late registration
            next_round = (
                Round.objects.filter(season=season, publish_pairings=False)
                .order_by("number")
                .first()
            )
            if next_round is not None:
                with reversion.create_revision():
                    reversion.set_user(request.user)
                    reversion.set_comment("Approved registration.")
                    PlayerLateRegistration.objects.update_or_create(
                        round=next_round,
                        player=player,
                        defaults={
        ...
        with reversion.create_revision():
            ...
            if created:
                ...
                if (
                    last_sp is not None
                    and last_sp.games_missed >= 2
                    and self.league.get_leaguesetting().carry_over_red_cards_as_yellow
                ):
                    sp.games_missed = 1
                    sp.save()
        ...
        for week_number in reg.weeks_unavailable.split(","):
            if week_number != "":
                round_ = Round.objects.filter(
                    season=season, number=int(week_number)
                ).first()
        ...
        if season.league.competitor_type == "team":
            subject = render_to_string(
                "tournament/emails/team_registration_approved_subject.txt", {"reg": reg}
            )
            msg_plain = render_to_string(
                "tournament/emails/team_registration_approved.txt", {"reg": reg}
            )
            msg_html = render_to_string(
                "tournament/emails/team_registration_approved.html", {"reg": reg}
            )
        elif season.league.rating_type == "blitz":
            # TODO: Make the email template a league setting
            subject = render_to_string(
                "tournament/emails/blitz_registration_approved_subject.txt",
                {"reg": reg},
            )
            msg_plain = render_to_string(
                "tournament/emails/blitz_registration_approved.txt", {"reg": reg}
            )
            msg_html = render_to_string(
                "tournament/emails/blitz_registration_approved.html", {"reg": reg}
            )
        else:
            subject = render_to_string(
                "tournament/emails/lone_registration_approved_subject.txt", {"reg": reg}
            )
            msg_plain = render_to_string(
                "tournament/emails/lone_registration_approved.txt", {"reg": reg}
            )
            msg_html = render_to_string(
                "tournament/emails/lone_registration_approved.html", {"reg": reg}
            )
            ...
            try:
                ...
                if modeladmin:
                    modeladmin.message_user(
                        request,
                        'Confirmation email sent to "%s".' % reg.email,
                        messages.INFO,
                    )
        ...
        if invite_to_slack:
            try:
                ...
                if modeladmin:
                    modeladmin.message_user(
                        request,
                        "The player is already in the slack group.",
                        messages.WARNING,
                    )
...
class MoveLateRegWorkflow:
    ...
    def run(self, update_fields):
        ...
        if update_fields and sp and sp.registration:
            subwf = ApproveRegistrationWorkflow(
                sp.registration, round_number=self.reg.round.number
            )
            next_round = subwf.default_section.round_set.filter(
                number=self.round.number + 1
            ).first()
            if not next_round:
                return False
