...
def not_updated_recently_usernames(active_usernames) -> List[str]:
    ...
    qs = players_qs.filter(date_modified__lte=_24_hours).exclude(
        lichess_username__in=active_usernames
    )
    ...
    for game in games_starting:
        try:
            ...
            for meta in lichessapi.get_latest_game_metas(
                game.white.lichess_username, 5, priority=1, timeout=300
            ):
                try:
                    if (
...
def validate_registration(reg_id):
    ...
    if not reg.can_commit and (
        reg.season.league.competitor_type != "team"
...
def validate_pending_registrations():
    ...
    regs_to_validate = Registration.objects.filter(
        season__registration_open=True, status__exact="pending"
    ).exclude(Q(validation_warning=False) & Q(validation_ok=True))
    ...
    for reg in regs_to_validate:
        signals.do_validate_registration.send(
            sender=validate_pending_registrations, reg_id=reg.pk
        )
