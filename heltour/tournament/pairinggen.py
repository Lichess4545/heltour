...
def _generate_team_pairings(round_, overwrite=False):
    with transaction.atomic():
        ...
        teams = Team.objects.filter(season=round_.season, is_active=True).select_related(
            'teamscore').nocache()
        for team in teams:
            if team.seed_rating is None:
                ...
                with reversion.create_revision():
                    ...
                    team.save()
        teams = sorted(teams, key=lambda team: team.get_teamscore().pairing_sort_key(),
                       reverse=True)

        previous_pairings = TeamPairing.objects.filter(round__season=round_.season,
                                                       round__number__lt=round_.number).order_by(
            'round__number')
...
def _get_player_list(team, round_, board_count):
    team_members = [TeamMember.objects.filter(team=team, board_number=b).first() for b in
                    range(1, board_count + 1)]
    alternates = list(
        AlternateAssignment.objects.filter(round=round_, team=team).order_by('board_number'))
...
def _generate_lone_pairings(round_, overwrite=False):
    with transaction.atomic():
        ...
        # Sort by seed rating/score
        season_players = SeasonPlayer.objects.filter(season=round_.season).select_related('player',
                                                                                          'loneplayerscore',
        ...
        include_players = {sp for sp in season_players if
                           sp.is_active and sp.player not in current_byes and sp.player not in unavailable_players}
        ...
        lone_pairings, byes = pairing_system.create_lone_pairings(round_, season_players,
                                                                  include_players,
                                                                  previous_pairings, previous_byes)
    ...
    def create_team_pairings(self, round_, teams, previous_pairings):
        ...
        for i in range(int(len(teams) / 2)):
            ...
            team_pairings.append(
                TeamPairing(white_team=white_team, black_team=black_team, round=round_,
                            pairing_order=i + 1))
...
class DutchTeamPairingSystem:
    def create_team_pairings(self, round_, teams, previous_pairings):
        ...
        players = [
            JavafoPlayer(team, team.teamscore.match_points,
                         list(self._process_pairings(team, previous_pairings))) for team in teams
        ]
    ...
    def _process_pairings(self, team, pairings):
        ...
        for p in team_pairings:
            if p.white_team == team:
                yield JavafoPairing(p.black_team, 'white',
                                    1.0 if p.white_points > p.black_points else 0.5 if p.white_points == p.black_points else 0)
            if p.black_team == team:
                yield JavafoPairing(p.white_team, 'black',
                                    1.0 if p.black_points > p.white_points else 0.5 if p.white_points == p.black_points else 0)
...
class DutchLonePairingSystem:
    ...
    def create_lone_pairings(self, round_, season_players, include_players, previous_pairings,
                             previous_byes):
        ...
        if self.accel == 'baku':
            ...
            if round_.number == 1:
                ...
                sorted_players = sorted(include_players, key=lambda sp: sp.seed_rating,
                                        reverse=True)
                ...
                min_rating_for_group_1 = min((sp.seed_rating for sp in season_players if
                                              sp.loneplayerscore.acceleration_group == 1))
                for sp in season_players:
        ...
        players = [
            JavafoPlayer(
                ...
                list(self._process_pairings(sp, previous_pairings, previous_byes, round_.number,
        ...
        for i in range(len(pairs)):
            ...
            if black is None:
                ...
                lone_pairings.append(
                    LonePlayerPairing(white=white, black=black, round=round_, pairing_order=i + 1))
        ...
        for n in range(1, current_round_number):
            ...
            if p is not None:
                if p.white == sp.player:
                    yield JavafoPairing(p.black, 'white', p.white_score(),
                                        forfeit=not p.game_played())
                else:
                    yield JavafoPairing(p.white, 'black', p.black_score(),
                                        forfeit=not p.game_played())
            elif b is not None:
...
class JavafoInstance:
        ...
        try:
            ...
            for n, player in enumerate(self.players, 1):
                ...
                for pairing in player.pairings:
                    opponent_num = next((num for num, player in enumerate(self.players, 1) if
                                         player.player == pairing.opponent), '0000')
    ...
    def _call_proc(self, input_file_name, output_file_name, args):
        proc = subprocess.Popen(
            '%s %s -p %s %s' % (settings.JAVAFO_COMMAND, input_file_name, output_file_name, args),
