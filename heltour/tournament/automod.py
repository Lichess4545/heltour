...
def mod_request_saved(instance, created, **kwargs):
    if created:
        signals.mod_request_created.send(
            sender=MOD_REQUEST_SENDER[instance.type], instance=instance
        )


@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["appeal_late_response"],
...
def appeal_late_response_created(instance, **kwargs):
    ...
    if not instance.round or instance.round.publish_pairings:
        instance.round = (
            instance.season.round_set.order_by("number")
            .filter(publish_pairings=True, is_completed=False)
            .first()
        )
        instance.save()
...
@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["request_continuation"],
    dispatch_uid="heltour.tournament.automod",
)
...
@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["withdraw"],
    dispatch_uid="heltour.tournament.automod",
)
def withdraw_created(instance, **kwargs):
    ...
    sp = SeasonPlayer.objects.filter(
        player=instance.requester, season=instance.season
    ).first()
    if sp is None:
        instance.reject(
            response="You aren't currently a participant in %s." % instance.season
        )
        return
    ...
    instance.approve(
        response="You've been withdrawn from the season. The withdrawal takes effect at the start of round %d."
        % instance.round.number
    )


@receiver(
    signals.mod_request_approved,
    sender=MOD_REQUEST_SENDER["withdraw"],
    dispatch_uid="heltour.tournament.automod",
)
def withdraw_approved(instance, **kwargs):
    ...
    with reversion.create_revision():
        reversion.set_comment(
            "Withdraw request approved by %s" % instance.status_changed_by
        )
        PlayerWithdrawal.objects.get_or_create(
            player=instance.requester, round=instance.round
        )
...
def automod_unresponsive(round_, **kwargs):
    ...
    for p in (
        round_.pairings.filter(game_link="", result="", scheduled_time=None)
        .exclude(white=None)
        .exclude(black=None)
    ):
        ...
        if round_.season.league.competitor_type == "team":
            white_unavail = PlayerAvailability.objects.filter(
                round=round_, player=p.white, is_available=False
            ).exists()
            black_unavail = PlayerAvailability.objects.filter(
                round=round_, player=p.black, is_available=False
            ).exists()
...
def player_unresponsive(round_, pairing, player, groups):
    ...
    has_warning = PlayerWarning.objects.filter(
        player=player, round__season=season, type="unresponsive"
    ).exists()
    if not has_warning and league.get_leaguesetting().warning_for_late_response:
        with reversion.create_revision():
            ...
            PlayerWarning.objects.get_or_create(
...
def appeal_late_response_approved(instance, **kwargs):
    ...
    with reversion.create_revision():
        reversion.set_comment(
            "Late response appeal approved by %s" % instance.status_changed_by
        )
        warning = PlayerWarning.objects.filter(
            player=instance.requester, round=instance.round, type="unresponsive"
        ).first()
        if warning:
            warning.delete()
        else:
...
def player_noshow(pairing, player, opponent):
    ...
    signals.notify_noshow.send(
        sender=automod_unresponsive, round_=round_, player=player, opponent=opponent
    )
...
def claim_win_noshow_created(instance, **kwargs):
    ...
    if not instance.round:
        instance.round = (
            instance.season.round_set.order_by("number")
            .filter(is_completed=False, publish_pairings=True)
            .first()
        )
        instance.save()
    ...
    if not instance.pairing:
        instance.reject(
            response="You don't currently have a pairing you can claim a win for."
        )
        return
...
def claim_win_noshow_approved(instance, **kwargs):
    ...
    punishment = "You have been given a %s card." % card_color
    allow_continue = (
        card_color != "red" and instance.season.league.competitor_type != "team"
    )
...
@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["appeal_noshow"],
    dispatch_uid="heltour.tournament.automod",
)
...
def appeal_noshow_approved(instance, **kwargs):
    ...
    with reversion.create_revision():
        reversion.set_comment(
            "No-show appeal approved by %s" % instance.status_changed_by
        )
...
@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["claim_draw_scheduling"],
    dispatch_uid="heltour.tournament.automod",
)
def claim_draw_scheduling_created(instance, **kwargs):
    ...
    add_system_comment(
        instance.pairing, "Scheduling draw claim made by %s" % instance.requester
    )
...
def claim_scheduling_draw_approved(instance, **kwargs):
    ...
    signals.notify_scheduling_draw_claim.send(
        sender=claim_scheduling_draw_approved, round_=instance.round, player=opponent
    )


@receiver(
    signals.mod_request_created,
    sender=MOD_REQUEST_SENDER["appeal_draw_scheduling"],
    dispatch_uid="heltour.tournament.automod",
)
def appeal_scheduling_draw_created(instance, **kwargs):
    ...
    if not instance.pairing and instance.round:
        ...
        instance.save()
    add_system_comment(
        instance.pairing, "Scheduling draw appeal by %s" % instance.requester
    )
...
def give_card(round_, player, type_):
    ...
    with transaction.atomic():
        ...
        if not sp:
            logger.error(
                "Season player did not exist for %s %s" % (round_.season, player)
            )
            return None
        already_has_card = PlayerWarning.objects.filter(
            player=player, round=round_, type__startswith="card"
        ).exists()
        card, _ = PlayerWarning.objects.get_or_create(
            player=player, round=round_, type=type_
        )
        if not already_has_card:
            sp.games_missed += 1
            with reversion.create_revision():
                reversion.set_comment(
                    "Automatic %s %s" % (sp.card_color, card.get_type_display())
                )
                sp.save()
...
def revoke_card(round_, player, type_):
    with transaction.atomic():
        ...
        if not sp:
            ...
            return
        card = PlayerWarning.objects.filter(
            player=player, round=round_, type=type_
        ).first()
        if not card:
            return
        card.delete()
        has_other_card = PlayerWarning.objects.filter(
            player=player, round=round_, type__startswith="card"
        ).exists()
