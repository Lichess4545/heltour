...
def require_api_token(view_func):
    def _wrapped_view_func(request, *args, **kwargs):
        ...
        if (
            match is None
            or len(ApiKey.objects.filter(secret_token=match.group(1))) == 0
        ):
            return HttpResponse("Unauthorized", status=401)
...
def _get_active_rounds(league_tag, season_tag):
    rounds = Round.objects.filter(
        season__is_active=True, publish_pairings=True, is_completed=False
    ).order_by("-season__start_date", "-season__id", "-number")
...
def _filter_pairings(pairings, player=None, white=None, black=None, scheduled=None):
    ...
    if white is not None:
        pairings = pairings.filter(
            white__lichess_username__iexact=white
        ) | pairings.filter(white__slack_user_id__iexact=white)
    if black is not None:
        pairings = pairings.filter(
            black__lichess_username__iexact=black
        ) | pairings.filter(black__slack_user_id__iexact=black)
    if scheduled is True:
        pairings = pairings.exclude(result="", scheduled_time=None)
    # TODO: else? or is not actually a boolean?
...
def _team_roster(season):
    ...
    all_alternates = sorted(
        Alternate.objects.filter(season_player__season=season)
        .select_related("season_player__player", "season_player__registration")
        .nocache(),
    ...
    players = sorted(
        {alt.season_player.player for alt in all_alternates}
        | {tm.player for tm in all_teammembers}
    )
            ...
            "teams": [
                {
                    ...
                    "players": [
                        ...
                        for team_member in all_teammembers
                        if team_member.team_id == team.pk
                    ],
                }
                for team in teams
            ],
            "alternates": [
                {
                    ...
                    "usernames": [
                        ...
                        if alt.board_number == board_number
                    ],
                }
...
def _lone_roster(season):
    ...
    player_board = {}
    current_round = season.round_set.filter(
...
def assign_alternate(request):
    ...
    alternate = Alternate.objects.filter(
        season_player__season=season,
        season_player__player=player,
        board_number=board_num,
    ).first()
    member_playing_up = team.teammember_set.filter(
        player=player, board_number__gte=board_num
    ).first()
    if alternate is None and member_playing_up is None:
        return JsonResponse({"updated": 0, "error": "not_an_alternate"})

    AlternateAssignment.objects.update_or_create(
        round=round_,
        team=team,
        board_number=board_num,
        defaults={"player": player, "replaced_player": None},
    )
...
def set_availability(request):
    ...
    try:
        ...
        season_player = SeasonPlayer.objects.filter(
            player=player, season=round_.season
        ).first()
    except IndexError:
        return JsonResponse({"updated": 0, "error": "no_matching_rounds"})
    ...
    PlayerAvailability.objects.update_or_create(
        round=round_, player=player, defaults={"is_available": is_available}
    )
...
def get_league_moderators(request):
    ...
    moderator_names = [
        lm.player.lichess_username
        for lm in LeagueModerator.objects.filter(league__tag=league_tag, is_active=True)
    ]
...
def league_document(request):
    ...
    league_doc = LeagueDocument.objects.filter(
        league__tag=league_tag, type=type_
    ).first()
    ...
    return JsonResponse({"name": document.name, "content": content})
...
def link_slack(request):
    ...
    already_linked = [
        p.lichess_username for p in Player.objects.filter(slack_user_id=user_id)
    ]

    return JsonResponse(
        {"url": url, "already_linked": already_linked, "expires": token.expires}
    )
...
def get_slack_user_map(request):
    users = {
        p.lichess_username: p.slack_user_id
        for p in Player.objects.exclude(slack_user_id="")
    }
...
def player_contact(request):
    ...
    time = timezone.now()
    for r in rounds:
        pairings = r.pairings.filter(
            white__lichess_username__iexact=sender,
            black__lichess_username__iexact=recip,
        ) | r.pairings.filter(
            white__lichess_username__iexact=recip,
...
def get_season_games(request):
    ...
    for s in seasons:
        for p in s.pairings.select_related(
            "white",
            "black",
