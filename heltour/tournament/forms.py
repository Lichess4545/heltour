...
from heltour import gdpr, settings
...
class RegistrationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        already_accepted = SeasonPlayer.objects.filter(
            season__in=self.season.section_list(),
            player__lichess_username=self.username,
        ).exists()
        ...
        self.fields["classical_rating"] = forms.IntegerField(
            required=True, label=_("Your Lichess %s Rating" % rating_type)
        )
        ...
        self.fields["has_played_20_games"] = forms.TypedChoiceField(
            ...
            help_text=_(help_text_provisional),
        )

        # In slack
        self.fields["already_in_slack_group"] = forms.TypedChoiceField(
            required=True,
            label=_("Are you in our Slack group?"),
            choices=YES_NO_OPTIONS,
            widget=forms.RadioSelect,
            coerce=lambda x: x == "True",
        )
        ...
        if league.competitor_type == "team":
            self.fields["previous_season_alternate"] = forms.ChoiceField(
                required=True,
        ...
        if league.rating_type != "blitz":
            self.fields["can_commit"] = forms.TypedChoiceField(
                ...
                label=_(
                    ...
                    % (time_control, league.rating_type)
                ),
            )
        else:
            start_time = (
                ""
                if self.season.start_date is None
                else " on %s at %s UTC"
                % (
        ...
        if league.competitor_type == "team":
            ...
            self.fields["avoid"] = forms.CharField(
                required=False,
                label=_("Are there any players you would like NOT to be paired with?"),
        ...
        if rules_doc is not None:
            ...
            rules_help_text = _(
                '<a target="_blank" href="%s">Rules Document</a>' % doc_url
            )
        else:
            rules_help_text = ""
        ...
        if round_to_open is not None:
            if is_team_league:
                self.fields["update_board_order"] = forms.BooleanField(
                    initial=True, required=False, label="Update board order"
                )
...
class NotificationsForm(forms.Form):
    def __init__(self, league, player, *args, **kwargs):
        ...
        for notifcation_type in PLAYER_NOTIFICATION_TYPES:
            type_ = notifcation_type[0]
