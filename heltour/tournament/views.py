...
class BaseView(View):
    ...
    def read_user_data(self):
        ...
        if self.request.user.is_authenticated:
            player_setting = PlayerSetting.objects.filter(
                player__lichess_username__iexact=self.request.user.username
            ).first()
            if player_setting:
        ...
        context.update(
            {
                ...
                "other_leagues": League.objects.filter(is_active=True)
                .order_by("display_order")
                .exclude(pk=self.league.pk),
            }
        )
        context.update(self.extra_context)
        return render(self.request, template, context)
...
class SeasonView(LeagueView):
    def get(self, request, *args, **kwargs):
        ...
        if not self._season_specified:
            return redirect(
                "by_league:by_season:%s" % request.resolver_match.url_name,
                *self.args,
...
class LeagueHomeView(LeagueView):
    ...
    def team_view(self):
        ...
        intro_doc = LeagueDocument.objects.filter(
            league=self.league, type="intro"
        ).first()
        ...
        if self.season is None:
            context = {
                ...
                "intro_doc": intro_doc,
                "can_edit_document": self.request.user.has_perm(
                    "tournament.change_document", self.league
                ),
        ...
        team_scores = list(
            enumerate(
                sorted(
                    TeamScore.objects.filter(team__season=self.season)
                    .select_related("team")
                    .nocache(),
                    reverse=True,
                )[:5],
...
class SeasonLandingView(SeasonView):
    ...
    def team_view(self):
        @cached_as(
            SeasonDocument, Document, TeamScore, TeamPairing, *common_team_models
        )
        def _view(league_tag, season_tag, user_data):
            ...
            last_round = (
                Round.objects.filter(season=self.season, is_completed=True)
                .order_by("-number")
                .first()
            )
            last_round_pairings = (
                last_round.teampairing_set.all() if last_round is not None else None
            )
            team_scores = list(
                enumerate(
                    sorted(
                        TeamScore.objects.filter(team__season=self.season), reverse=True
                    )[:5],
                    1,
                )
            )

            links_doc = SeasonDocument.objects.filter(
                season=self.season, type="links"
            ).first()
    ...
    def lone_view(self):
        ...
        def _view(league_tag, season_tag, user_data):
            ...
            last_round_pairings = (
                last_round.loneplayerpairing_set.exclude(result="")
                .order_by("pairing_order")[:10]
                .nocache()
    ...
    def lone_completed_season_view(self):
        ...
        ribbons = SeasonPrizeWinner.objects.filter(
            season_prize__season=self.season,
...
class PairingsView(SeasonView):
    ...
    def _player_status(
        self, player, pairing, presences, in_contact_period, contact_deadline
    ):
        if player is None:
            return (None, "no player")
        if (player is pairing.white and pairing.white_confirmed) or (
            player is pairing.black and pairing.black_confirmed
        ):
    ...
    def get_team_context(
        self, league_tag, season_tag, round_number, team_number, can_change_pairing
    ):
        specified_round = round_number is not None
        round_number_list = [
            round_.number
            for round_ in Round.objects.filter(
                season=self.season, publish_pairings=True
            ).order_by("-number")
        ]
        ...
        team_pairings = (
            TeamPairing.objects.filter(
                round__number=round_number, round__season=self.season
            )
        ...
        if team_number is not None:
            ...
            team_pairings = team_pairings.filter(
                white_team=current_team
            ) | team_pairings.filter(black_team=current_team)
        else:
            current_team = None
        ...
        presences = {
            (pp.player_id, pp.pairing_id): pp
            for pp in PlayerPresence.objects.filter(round=round_)
        }
        if pairing_lists:
            contact_deadline = (
                round_.start_date + self.league.get_leaguesetting().contact_period
            )
        ...
        pairing_lists = [
            [
                (
                    (p,)
                    + status(p.white_team_player(), p)
                    + status(p.black_team_player(), p)
                )
                for p in p_list
            ]
        ...
        unavailable_players = {
            pa.player
            for pa in PlayerAvailability.objects.filter(
        ...
        captains = {
            ...
            for tm in TeamMember.objects.filter(
                team__season=self.season, is_captain=True
            )
        }
        ...
        show_legend = (
            len(
                unavailable_players
                & (
                    {p[0].white for plist in pairing_lists for p in plist}
                    | {p[0].black for plist in pairing_lists for p in plist}
                )
            )
            > 0
        )
    ...
    def team_view(self, round_number=None, team_number=None):
        ...
        def _view(
            ...
            context = self.get_team_context(
                league_tag, season_tag, round_number, team_number, can_change_pairing
            )
    ...
    def get_lone_context(self, round_number=None, team_number=None):
        ...
        round_ = Round.objects.filter(number=round_number, season=self.season).first()
        pairings = (
            LonePlayerPairing.objects.filter(round=round_)
            .order_by("pairing_order")
            .select_related("white", "black")
            .nocache()
        )
        ...
        active_players = {
            sp.player
            for sp in SeasonPlayer.objects.filter(season=self.season, is_active=True)
        }
        ...
        # Add errors
        pairings = [
            (p, pairing_error(p)) + status(p.white, p) + status(p.black, p)
            for p in pairings
        ]
...
class ICalPairingsView(PairingsView, ICalMixin):
    ...
    def team_view(self, round_number=None, team_number=None):
        ...
        return self.ical_from_pairings_list(
            full_pairings_list, calendar_title, uid_component
        )
...
class RegisterView(LoginRequiredMixin, LeagueView):
    def view(self, post=False):
        ...
        with cache.lock(
            f"update_create_registration-{self.request.user.id}-{reg_season.id}"
        ):
            instance = Registration.get_latest_registration(
                self.request.user, reg_season
            )
            if post:
                ...
                if form.is_valid():
                    ...
                    msg_plain = render_to_string(
                        "tournament/emails/registration_received.txt",
                        {"reg": form.instance},
                    )
                    msg_html = render_to_string(
                        "tournament/emails/registration_received.html",
                        {"reg": form.instance},
                    )
                    try:
                        send_mail(
                ...
                if rules_doc is not None:
                    doc_url = reverse(
                        "by_league:document", args=[self.league.tag, rules_doc.tag]
                    )
                else:
                    doc_url = ""
                form = RegistrationForm(
    ...
    def view(self, req_type, post=False):
        ...
        if post:
            ...
            if form.is_valid():
                ...
                return redirect(
                    leagueurl(
                        "modrequest_success", self.league.tag, self.season.tag, req_type
                    )
                )
        else:
            form = ModRequestForm()
...
class ModRequestSuccessView(SeasonView):
    def view(self, req_type):
        context = {"req_type": ModRequest(type=req_type).get_type_display()}
        ...
        def _view(league_tag, season_tag, user_data, can_edit):
            ...
            if self.season is None:
                context = {
                    "can_edit": self.request.user.has_perm(
                        "tournament.manage_players", self.league
                    ),
                }
                return self.render("tournament/team_rosters.html", context)

            teams = (
                Team.objects.filter(season=self.season)
                .order_by("number")
                .prefetch_related(
                    Prefetch(
                        "teammember_set",
                        queryset=TeamMember.objects.select_related("player"),
                    )
                )
                .nocache()
            )
            ...
            alternates_by_board = [
                sorted(
                    ...
                    .select_related(
                        "season_player__registration", "season_player__player"
                    )
                    .nocache(),
            ...
            alternate_rows = list(
                enumerate(itertools.zip_longest(*alternates_by_board), 1)
            )
            if len(alternate_rows) == 0:
                alternate_rows.append((1, [None for _ in board_numbers]))

            current_round = (
                Round.objects.filter(season=self.season, publish_pairings=True)
                .order_by("-number")
                .first()
            )
            scheduled_alternates = {
                assign.player
                for assign in AlternateAssignment.objects.filter(round=current_round)
                .select_related("player")
                .nocache()
            }
            unavailable_players = {
                pa.player
                for pa in PlayerAvailability.objects.filter(
                    round__season=self.season, round=current_round, is_available=False
                )
                .select_related("player")
                .nocache()
            }
            unresponsive_players = {
                sp.player
                for sp in SeasonPlayer.objects.filter(
                    season=self.season, unresponsive=True
                )
                .select_related("player")
                .nocache()
            }
            games_missed_by_player = {
                sp.player: sp.games_missed
                for sp in SeasonPlayer.objects.filter(season=self.season)
                .select_related("player")
                .nocache()
            }
            yellow_card_players = {
                player
                for player, games_missed in list(games_missed_by_player.items())
                if games_missed == 1
            }
            red_card_players = {
                player
            ...
            if section is not None:
                ...
                if match is not None:
                    ...
                    player_scores = [
                        ...
                        if ps[1].season_player.seed_rating_display() is not None
                        and ps[1].season_player.seed_rating_display() < max_rating
                    ]

            player_sections = [
                ("u%d" % sp.max_rating, "U%d" % sp.max_rating)
                for sp in SeasonPrize.objects.filter(season=self.season)
                .exclude(max_rating=None)
                .order_by("max_rating")
            ]
...
def _get_player_highlights(prize_winners):
    return [
        (
            ...
            {
                pw.player
                for pw in prize_winners.filter(
                    season_prize__rank=1, season_prize__max_rating=None
                )
            },
        ),
        (
            "silver",
            {
                pw.player
                for pw in prize_winners.filter(
                    season_prize__rank=2, season_prize__max_rating=None
                )
            },
        ),
        (
            "bronze",
            {
                pw.player
                for pw in prize_winners.filter(
                    season_prize__rank=3, season_prize__max_rating=None
                )
            },
        ),
        (
            ...
            {
                pw.player
                for pw in prize_winners.filter(season_prize__rank=1).exclude(
...
@cached_as(
    LonePlayerScore, LonePlayerPairing, PlayerPairing, PlayerBye, SeasonPlayer, Player
)
def _lone_player_scores(season, final=False, sort_by_seed=False, include_current=False):
    ...
    if sort_by_seed:
        ...
        def sort_key(s):
            return s.season_player.seed_rating_display() or 0
    elif season.is_completed or final:

        def sort_key(s):
            return s.final_standings_sort_key()
    else:

        def sort_key(s):
            return s.intermediate_standings_sort_key()

    raw_player_scores = (
        LonePlayerScore.objects.filter(season_player__season=season)
        .select_related("season_player__player", "season_player__season__league")
        .nocache()
    )
    player_scores = list(
        enumerate(sorted(raw_player_scores, key=sort_key, reverse=True), 1)
    )
    ...
    byes = (
        PlayerBye.objects.filter(round__season=season)
        .select_related("round", "player")
        .nocache()
    )
    ...
    def view(self):
        ...
        def _view(league_tag, season_tag, user_data):
            if self.league.is_team_league():
                raise Http404
            round_numbers = list(range(1, self.season.rounds + 1))
            player_scores = _lone_player_scores(
                self.season, sort_by_seed=True, include_current=True
            )
...
class StatsView(SeasonView):
        ...
        def _view(league_tag, season_tag, user_data):
            ...
            def count_results(board_num=None):
                ...
                for p in all_pairings:
                    if (
                        board_num is not None
                        and p.teamplayerpairing.board_number != board_num
                    ):
                        continue
                    ...
                    total += 1
                    if (
                        p.white_rating_display(self.league) is not None
                        and p.black_rating_display(self.league) is not None
                    ):
                        rating_delta += p.white_rating_display(
                            self.league
                        ) - p.black_rating_display(self.league)
                    if p.result == "1-0":
                        counts[0] += 1
                        counts[3] += 1
                    elif p.result == "0-1":
            ...
            boards = [
                count_results(board_num=n) for n in self.season.board_number_list()
            ]
    ...
    def lone_view(self):
        @cached_as(
            League, Season, Round, LonePlayerPairing, PlayerPairing, SeasonPlayer
        )
        def _view(league_tag, season_tag, user_data):
            season_players = (
                self.season.seasonplayer_set.order_by("player__rating")
                .select_related("player")
                .nocache()
            )
            active_player_ratings = [
                sp.player.player_rating_display(self.league)
                for sp in season_players.filter(is_active=True)
            ]
            active_player_ratings = [r for r in active_player_ratings if r is not None]
            all_player_ratings = [
                sp.player.player_rating_display(self.league) for sp in season_players
            ]
            ...
            for p in all_pairings:
                ...
                if (
                    ...
                    d = p.white_rating_display(self.league) - p.black_rating_display(
                        self.league
                    )
                    rating_delta += d
                    ...
                    if math.copysign(
                        1,
                        p.white_rating_display(self.league)
                        - p.black_rating_display(self.league),
                    ) == p.black_score() - p.white_score() and p.white_rating_display(
                        self.league
                    ) != p.black_rating_display(self.league):
            ...
            upset_percents = tuple(
                (
                    c1 / float(c2) if c2 > 0 else 0
                    for c1, c2 in zip(upset_counts, rating_delta_counts)
                )
            )
...
class ActivePlayerTableView(LeagueView):
    ...
    def view(self, page: int = 1):
        tablesums = (
            Player.objects.only("lichess_username")
            .annotate(
                blackcount=Count(
                    "pairings_as_black", filter=self.black_games_Q(), distinct=True
                ),
                whitecount=Count(
                    "pairings_as_white", filter=self.white_games_Q(), distinct=True
                ),
                game_count=F("blackcount") + F("whitecount"),
                season_count=Count(
                    "seasonplayer", filter=self.seasons_Q(), distinct=True
                ),
        ...
        def _view(league_tag, season_tag, user_data, board_number):
            ...
            for pairing in board_pairings:
                ...
                if pairing.teamplayerpairing.board_number != int(board_number):
                    continue
                white_ps = ps_dict[pairing.white] = ps_dict.get(
                    pairing.white, PlayerScore(pairing.white)
                )
                black_ps = ps_dict[pairing.black] = ps_dict.get(
                    pairing.black, PlayerScore(pairing.black)
                )
                ...
                if white_game_score is not None:
                    ...
                    if pairing.game_played():
                        white_ps.perf.add_game(
                            white_game_score, pairing.black_rating_display(self.league)
                        )
                ...
                if black_game_score is not None:
                    ...
                    if pairing.game_played():
                        black_ps.perf.add_game(
                            black_game_score, pairing.white_rating_display(self.league)
                        )
                    ...
                    for g in games_dict[ps.player]:
                        if g.game_played():
                            if g.white == ps.player:
                                ps.perf.add_game(
                                    g.white_score(), g.black_rating_display(self.league)
                                )
                            else:
                                ps.perf.add_game(
                                    g.black_score(), g.white_rating_display(self.league)
                                )
                    ps.perf_rating = ps.perf.calculate()
                    ps.eligible = False
                return True
            ...
            ps_list.sort(
                key=lambda ps: (ps.perf_rating or 0, ps.score, -ps.score_total),
                reverse=True,
            )

            context = {"board_number": board_number, "player_scores": ps_list}
...
class LeagueDashboardView(LeagueView):
    ...
    def _common_context(self):
        current_season_list, completed_season_list = _get_season_lists(
            self.league, active_only=False
        )
        ...
        pending_reg_count = len(
            Registration.objects.filter(season=reg_season, status="pending")
        )
        pending_modreq_count = len(
            ModRequest.objects.filter(season=self.season, status="pending")
        )
        ...
        alternates = (
            Alternate.objects.filter(season_player__season=self.season)
        ...
        alternate_search_count = (
            len(alternates_manager.current_searches(alternate_search_round))
            if alternate_search_round is not None
            else None
        )

        last_round = (
            Round.objects.filter(
                season=self.season, publish_pairings=True, is_completed=False
            )
            .order_by("number")
            .first()
        )
        next_round = (
            Round.objects.filter(
                season=self.season, publish_pairings=False, is_completed=False
            )
            .order_by("number")
            .first()
        )
        ...
        return {
            ...
            "can_view_dashboard": self.request.user.has_perm(
                "tournament.view_dashboard", self.league
            ),
...
class UserDashboardView(LeagueView):
    def view(self):
        ...
        active_seasons = self.league.season_set.filter(is_completed=False).order_by(
            "-start_date"
        )
        active_seasons_with_sp = [
            (s, player.seasonplayer_set.filter(season=s).first())
            for s in active_seasons
        ]
        active_seasons_with_sp = [s for s in active_seasons_with_sp if s[1]]
        last_sp = (
            player.seasonplayer_set.filter(
        ...
        active_rounds = Round.objects.filter(
            publish_pairings=True, is_completed=False, season__is_active=True
        )

        approved = Registration.objects.filter(
            lichess_username=self.request.user.username, status="approved"
        ).exists()
        ...
        for r in active_rounds:
            my_pairings += [
                (r, p)
                for p in r.pairings.filter(white=player).exclude(black=None)
                | r.pairings.filter(black=player).exclude(white=None)
            ]
...
class DocumentView(LeagueView):
    def view(self, document_tag):
        league_document = LeagueDocument.objects.filter(
            league=self.league, tag=document_tag
        ).first()
        if league_document is None:
            season_document = SeasonDocument.objects.filter(
                season=self.season, tag=document_tag
            ).first()
        ...
        if post:
            ...
            if form.is_valid():
                ...
                for mod in league.leaguemoderator_set.all():
                    if (
                        mod.send_contact_emails
                        and mod.player.email
                        and not form_contains_links
                    ):
                        ...
                        message.send()
                return redirect(
                    leagueurl("contact_success", league_tag=self.league.tag)
                )
        else:
...
class PlayerProfileView(LeagueView):
    def view(self, username):
        ...
        def game_count(season):
            ...
            return (
                season_pairings.filter(white=player)
        ...
        leagues = list(
            (
                League.objects.filter(is_active=True)
                | League.objects.filter(pk=self.league.pk)
            ).order_by("display_order")
        )
        ...
        other_season_leagues = [
            (
                league,
                [
                    (sp.season, game_count(sp.season), team(sp.season))
                    for sp in player.seasonplayer_set.filter(
                        season__league=league, season__is_active=True
                    ).order_by("-season__start_date")
                ],
            )
        ...
        other_season_leagues = [
            league for league in other_season_leagues if len(league[1]) > 0
        ]

        season_player = SeasonPlayer.objects.filter(
        ...
        def season_performance(season, isCurrentSeason=False):
            ...
            if season is None:
                ...
                pairings = TeamPlayerPairing.objects.filter(
                    white=player
                ) | TeamPlayerPairing.objects.filter(black=player)
                for p in (
                    pairings.filter(team_pairing__round__season=season)
                    .order_by("team_pairing__round__number")
                    .nocache()
                ):
                ...
                pairings = LonePlayerPairing.objects.filter(
                    white=player
                ) | LonePlayerPairing.objects.filter(black=player)
                for p in (
                    pairings.filter(round__season=season)
                ...
                byes = {
                    ...
                    for b in PlayerBye.objects.filter(
                        round__season=season, player=player
                    )
                }

            history = []
            for round_ in season.round_set.filter(publish_pairings=True).order_by(
                "number"
            ):
                if round_.number in games:
                    for p in games[round_.number]:
                        ...
                        game_score = (
                            p.white_score() if p.white == player else p.black_score()
                        )
                        ...
                        if (
                            p.game_played()
                            and p.white is not None
                            and p.black is not None
                        ):
                            sp = SeasonPlayer.objects.filter(
                                season=season,
                                player=p.black if p.white == player else p.white,
                            ).first()
                            if sp is not None and sp.seed_rating is not None:
                                opp_rating = sp.seed_rating
                            else:
                                opp_rating = (
                                    p.black_rating_display()
                                    if p.white == player
                                    else p.white_rating_display(self.league)
                                )
        ...
        season_score, season_score_total, season_perf, history, games, byes = (
            season_performance(self.season, isCurrentSeason=True)
        )
        ...
        for season in [
            sp.season
            for sp in player.seasonplayer_set.filter(season__league=self.league)
        ]:
            part_career_score, part_career_score_total, part_career_perf, _, _, _ = (
        ...
        team_member = TeamMember.objects.filter(
            team__season=self.season, player=player
        ).first()
        alternate = Alternate.objects.filter(season_player=season_player).first()

        schedule = []
        for round_ in self.season.round_set.filter(is_completed=False).order_by(
            "number"
        ):
            ...
            if self.season.league.is_team_league():
                assignment = AlternateAssignment.objects.filter(
                ...
                if round_.number in byes:
                    if round_.publish_pairings:
                        continue
                    schedule.append(
                        (round_, None, byes[round_.number].get_type_display(), None)
                    )
                    continue
                if not player.is_available_for(round_):
                    schedule.append((round_, None, "Unavailable", None))
        ...
        context = {
            ...
            "can_edit": self.request.user.has_perm(
                "tournament.change_season_player", self.league
            ),
            "trophies": trophies,
...
class TeamProfileView(LeagueView):
    def view(self, team_number):
        ...
        prev_members = [
            (
                player,
                display_ratings.get(player, None) or player.rating_for(self.league),
                game_count,
            )
            for player, game_count in sorted(
                list(game_counts.items()), key=lambda i: i[0].lichess_username.lower()
            )
            if player not in member_players
        ]

        matches = []
        for round_ in self.season.round_set.filter(publish_pairings=True).order_by(
            "number"
        ):
            if self.season.league.is_team_league():
    ...
    def view(self, post=False):
        ...
        if player is not None:
            if self.league.get_leaguesetting().limit_game_nominations_to_participants:
                player_pairings = season_pairings.filter(
                    white=player
                ) | season_pairings.filter(black=player)
            ...
            if can_nominate and self.season.nominations_open:
                current_nominations = GameNomination.objects.filter(
                    season=self.season, nominating_player=player
                )
...
class DeleteNominationView(LoginRequiredMixin, SeasonView):
    def view(self, nomination_id, post=False):
        ...
        if not self.season.nominations_open:
            return redirect(
                "by_league:by_season:nominate", self.league.tag, self.season.tag
            )

        nomination_to_delete = GameNomination.objects.filter(
            season=self.season, nominating_player=self.player, pk=nomination_id
        ).first()
...
class ScheduleView(LoginRequiredMixin, LeagueView):
    def view(self, post=False):
        times = (
            self.player.availabletime_set.filter(league=self.league)
...
class ConfirmScheduledTimeView(LoginRequiredMixin, LeagueView):
    def view(self, post=False):
        ...
        for r in active_rounds:
            next_pairings += [
                (r, p)
                for p in r.pairings.filter(white=player).exclude(scheduled_time=None)
                | r.pairings.filter(black=player).exclude(scheduled_time=None)
            ]

        context = {"player": player, "next_pairings": next_pairings}
...
class AvailabilityView(LoginRequiredMixin, SeasonView):
    def view(self, post=False):
        ...
        if (
            ...
            section_list = self.season.section_list()
            active_sp = player.seasonplayer_set.filter(
                season__in=section_list, is_active=True
            ).first()
            if active_sp:
                return redirect(
            ...
            inactive_sp = player.seasonplayer_set.filter(
                season__in=section_list
            ).first()
            if inactive_sp:
                return redirect(
                    "by_league:by_season:edit_availability",
        ...
        if include_current_round:
            round_list = list(
                self.season.round_set.order_by("number").filter(is_completed=False)
            )
        else:
            round_list = list(
        ...
        if player is not None:
            ...
            team_member = TeamMember.objects.filter(
                player=player, team__season=self.season
            ).first()
            if team_member is not None and (
                team_member.is_captain or team_member.is_vice_captain
            ):
                team = team_member.team
                for tm in (
                    team.teammember_set.order_by("board_number")
                    .select_related("player")
                    .nocache()
                ):
            ...
            availability_set = PlayerAvailability.objects.filter(
                player__in=player_list, round__in=round_list
            ).nocache()
            is_available_dict = {
                (av.round_id, av.player_id): av.is_available for av in availability_set
            }

            season_player_set = SeasonPlayer.objects.filter(
                player__in=player_list, season=self.season
            ).nocache()
            has_red_card_dict = {
                sp.player: sp.card_color == "red" for sp in season_player_set
            }
            ...
            if self.league.is_team_league() and len(round_list) > 0:
                ...
                game_is_scheduled_dict = {
                    (round_list[0], sp.player): sp.has_scheduled_game_in_round(
                        round_list[0]
                    )
                    for sp in season_player_set
                }
            else:
                game_is_scheduled_dict = {}
            ...
            if post:
                for r in round_list:
                    for p in player_list:
                        ...
                        season_player = SeasonPlayer.objects.filter(
                            player=p, season=self.season
                        ).first()
                        can_update_availability = (
                            season_player is not None
                            and season_player.card_color != "red"
                            and not game_is_scheduled_dict.get((r, p), False)
                        )

                        if (
                            is_available != is_available_dict.get((r.id, p.id), True)
                            and can_update_availability
                        ):
            ...
            round_data = [
                (
                    ...
                    [
                        (
                            p,
                            is_available_dict.get((r.id, p.id), True),
                            has_red_card_dict.get(p, False)
                            or game_is_scheduled_dict.get((r, p), False),
                        )
                        for p in player_list
                    ],
                )
    ...
    def set_league_and_season(self, league_tag, season_tag):
        ...
        if self.request.user.is_authenticated:
            ...
            active_sp = (
                self.player.seasonplayer_set.filter(
                    season__in=league_seasons, is_active=True
                )
                .order_by(
                    "-season__start_date", "season__section__order", "-season__id"
                )
                .first()
            )
            if active_sp:
    ...
    def view(self):
        ...
        def team_member(s):
            return TeamMember.objects.filter(
                team=s.team, board_number=s.board_number
            ).first()

        open_spots = [
            (s.board_number, s.team, team_member(s), s.date_created) for s in searches
        ]
        filled_spots = [
            (aa.board_number, aa.team, aa.player, aa.date_modified)
        ...
        def with_status(alt):
            ...
            if status == "Waiting":
                if alt.season_player.games_missed >= 2:
                    ...
                    round_.pairings.filter(white=alt.season_player.player)
                    | round_.pairings.filter(black=alt.season_player.player)
                ).exists():
                    status = "Scheduled"
        ...
        def alternate_board(n):
            ...
            eligible_alts = [
                (alt, status, date)
                for alt, status, date in alts_with_status
                if status in ("Waiting", "Contacted")
            ]
            ineligible_alts = [
                (alt, status, date)
                for alt, status, date in alts_with_status
                if status not in ("Waiting", "Contacted")
            ]
        ...
        alternates_by_board = [
            alternate_board(n) for n in self.season.board_number_list()
        ]
...
class AlternateAcceptView(LoginRequiredMixin, SeasonView):
    def view(self, round_number, post=False):
        ...
        alt = Alternate.objects.filter(
            season_player__season=self.season, season_player__player=self.player
        ).first()
        show_button = False
        if alt is None:
            ...
            if post:
                if alternates_manager.alternate_accepted(alt):
                    msg = (
                        "You have been assigned to a team for round %d. Please check Slack for more info."
                        % round_.number
                    )
                else:
                    msg = (
                        "Sorry, no games are currently available for round %d."
                        % round_.number
                    )
            else:
                start_time = (
                    round_.start_date.strftime("%Y-%m-%d %H:%M")
                    if not round_.publish_pairings
                    else "now"
                )
                end_time = round_.end_date.strftime("%Y-%m-%d %H:%M")
                msg = (
                    "Please confirm you can play a game for round %d. You must have multiple times you can play between %s and %s (UTC)."
                    % (round_.number, start_time, end_time)
                )
                show_button = True
        else:
            msg = (
                "Sorry, no games are currently available for round %d." % round_.number
            )

        context = {"msg": msg, "show_button": show_button}
    ...
    def view(self, round_number, post=False):
        ...
        if alt is None:
            ...
            if post:
                ...
                msg = (
                    "You will not receive any more game offers for round %d. Thank you for your response."
                    % round_.number
                )
            else:
                msg = (
                    "Please confirm you do not want to play a game during round %d."
                    % (round_.number)
                )
                show_button = True
        ...
        if post:
            ...
            if form.is_valid():
                ...
                for type_, _ in PLAYER_NOTIFICATION_TYPES:
                    ...
                    setting.offset = (
                        timedelta(minutes=form.cleaned_data[type_ + "_offset"])
                        if (type_ + "_offset") in form.cleaned_data
                        else None
                    )
                    setting.save()
        else:
...
class LoginView(LeagueView):
    def view(self, secret_token=None):
        return oauth.redirect_for_authorization(
            self.request, self.league.tag, secret_token
        )
...
class LoginFailedView(BaseView):
    def view(self):
        return self.render(
            "tournament/login_failed.html",
            {"lichess": settings.LICHESS_NAME, "lichess_link": settings.LICHESS_DOMAIN},
        )


class OAuthCallbackView(View):
    def get(self, request, *args, **kwargs):
        return oauth.login_with_code(
...
class TvView(LeagueView):
    def view(self):
        ...
        boards = (
            active_season.board_number_list()
            if active_season is not None and active_season.boards is not None
            else None
        )
        teams = (
            active_season.team_set.order_by("name")
            if active_season is not None
            else None
        )

        filter_form = TvFilterForm(
            current_league=self.league, leagues=leagues, boards=boards, teams=teams
        )
...
def _tv_json(league, board=None, team=None):
    def export_game(game, league, board, team):
        if hasattr(game, "teamplayerpairing"):
            ...
            return {
                ...
                "time": game.scheduled_time.isoformat()
                if game.scheduled_time is not None
                else None,
                ...
                "matches_filter": (
                    league is None and game_league.is_active or league == game_league
                )
                and (board is None or board == game.teamplayerpairing.board_number)
                and
                # TODO: Team filter can do weird things if there are multiple active seasons
                (
                    team is None
                    or team == game.teamplayerpairing.white_team().number
                    or team == game.teamplayerpairing.black_team().number
                ),
            }
    ...
    current_games = (
        ...
        .exclude(tv_state="hide")
        .exclude(game_link="")
        .order_by("scheduled_time")
        .select_related(
            "teamplayerpairing__team_pairing__round__season__league",
    ...
    scheduled_games = (
        PlayerPairing.objects.filter(
            result="",
            game_link="",
            scheduled_time__gt=timezone.now() - timedelta(minutes=20),
        )
        .order_by("scheduled_time")
        .select_related(
            "teamplayerpairing__team_pairing__round__season__league",
...
def _get_default_season(league_tag, allow_none=False):
    season = (
        Season.objects.filter(league=_get_league(league_tag), is_active=True)
        .order_by("-start_date", "section__order", "-id")
        .first()
    )
...
def _get_season_lists(league, active_only=True):
    season_list = Season.objects.filter(league=league).order_by(
        "-start_date", "section__order", "-id"
    )
    if active_only:
...
def _get_nav_tree(league_tag, season_tag):
    ...
    def transform(item):
        ...
        children = [
            transform(child) for child in all_items if child.parent_id == item.id
        ]
