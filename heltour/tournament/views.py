    ...
    def view(self, post=False):
        ...
        with cache.lock(
            ...
            if post:
                ...
                form = RegistrationForm(
                    instance=instance, season=reg_season, user=self.request.user
                )
                form.fields["email"].initial = player.email
                form.fields["classical_rating"].initial = player.rating_for(
                    reg_season.league
                )
                form.fields["has_played_20_games"].initial = not player.provisional_for(
                    reg_season.league
                )
                form.fields["already_in_slack_group"].initial = (
                    player.slack_user_id != ""
                )

            context = {"form": form, "registration_season": reg_season}
...
class ToggleDarkModeView(BaseView):
    def view(self):
        ...
        # FIXME: catching naked exceptions like this without logging is a footgun
        except Exception:
            pass
...
def _tv_json(league, board=None, team=None):
    ...
    current_games = (
        PlayerPairing.objects.filter(result="", tv_state="default")
        .exclude(game_link="")
