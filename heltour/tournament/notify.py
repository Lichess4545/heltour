...
def _send_notification(notification_type, league, text):
    if league.enable_notifications:
        for ln in league.leaguechannel_set.filter(
            type=notification_type, send_messages=True
        ):
            try:
...
def registration_saved(instance, created, **kwargs):
    ...
    reg_url = abs_url(
        reverse("admin:review_registration", args=[instance.pk])
        + "?_changelist_filters=status__exact%3Dpending%26season__id__exact%3D"
        + str(instance.season.pk)
    )
    list_url = abs_url(
        reverse("admin:tournament_registration_changelist")
        + "?status__exact=pending&season__id__exact="
        + str(instance.season.pk)
    )
    pending_count = instance.season.registration_set.filter(
        status="pending", season=instance.season
    ).count()
    message = "@%s (%s) has <%s|registered> for %s. <%s|%d pending>" % (
        instance.lichess_username,
        instance.classical_rating,
        reg_url,
        league.name,
        list_url,
        pending_count,
    ...
    pre_season = (
        instance.season.start_date and timezone.now() < instance.season.start_date
    )
    setting = league.get_leaguesetting()
    if (
        not pre_season
        and setting.notify_for_registrations
        or pre_season
        and setting.notify_for_pre_season_registrations
    ):
        _send_notification("mod", league, message)


@receiver(
    post_save, sender=PlayerLateRegistration, dispatch_uid="heltour.tournament.notify"
)
def latereg_saved(instance, created, **kwargs):
    ...
    manage_url = abs_url(
        reverse("admin:manage_players", args=[instance.round.season.pk])
    )
    message = "@%s <%s|added> for round %d" % (
        instance.player,
        manage_url,
        instance.round.number,
    )
...
def pairing_forfeit_changed(instance, **kwargs):
    ...
    white = (
        instance.white.lichess_username.lower() if instance.white is not None else "?"
    )
    black = (
        instance.black.lichess_username.lower() if instance.black is not None else "?"
    )
...
@receiver(
    signals.player_account_status_changed, dispatch_uid="heltour.tournament.notify"
)
def player_account_status_changed(instance, old_value, new_value, **kwargs):
    ...
    season_players = instance.seasonplayer_set.select_related(
        "season__league"
    ).nocache()
    pending_regs = (
        Registration.objects.filter(
            lichess_username__iexact=instance.lichess_username, status="pending"
        )
        .select_related("season__league")
        .nocache()
    )
    league_set = {sp.season.league for sp in season_players} | {
        reg.season.league for reg in pending_regs
    }
    for league in league_set:
        latest_season = (
            league.season_set.filter(is_active=True)
        ...
        if latest_season is not None:
            player_profile_url = abs_url(
                reverse(
                    ...
                    args=[league.tag, latest_season.tag, instance.lichess_username],
                )
            )
        else:
            player_profile_url = abs_url(
                reverse(
                    ...
                    args=[league.tag, instance.lichess_username],
                )
            )
        if old_value == "normal":
            message = "@%s marked as %s on <%s|lichess>. <%s|Player profile>" % (
                _slack_user(instance),
                new_value,
                lichess_profile_url,
                player_profile_url,
            )
            ...
            message = (
                "@%s <%s|lichess> account status changed from %s to %s. <%s|Player profile>"
                % (
                    _slack_user(instance),
                    lichess_profile_url,
                    old_value,
...
def notify_mods_unscheduled(round_, **kwargs):
    unscheduled_pairings = (
        round_.pairings.filter(result="", scheduled_time=None)
        .exclude(white=None)
        .exclude(black=None)
        .nocache()
    )
    if len(unscheduled_pairings) == 0:
        ...
        pairing_strs = (
            "@%s vs @%s"
            % (p.white.lichess_username.lower(), p.black.lichess_username.lower())
            for p in unscheduled_pairings
        )
        message = "%s - The following games are unscheduled: %s" % (
            round_,
            ", ".join(pairing_strs),
        )
...
def notify_mods_pending_regs(round_, **kwargs):
    pending_count = round_.season.registration_set.filter(
        status="pending", season=round_.season
    ).count()
    if pending_count == 0:
        return
    list_url = abs_url(
...
@receiver(
    signals.notify_mods_pairings_published, dispatch_uid="heltour.tournament.notify"
)
...
@receiver(
    signals.notify_mods_round_start_done, dispatch_uid="heltour.tournament.notify"
)
...
def pairings_generated(round_, **kwargs):
    ...
    message = "Pairings generated for round %d. <%s|Review>" % (
        round_.number,
        review_url,
    )
    _send_notification("mod", league, message)
...
def publish_scheduled(round_id, eta, **kwargs):
    ...
    message = "%s pairings will be published in %d minutes." % (
        round_,
        (eta - timezone.now()).total_seconds() / 60,
    )
    _send_notification("mod", round_.season.league, message)
...
def alternate_search_started(season, team, board_number, round_, **kwargs):
    ...
    if team_pairing is not None:
        pairing = (
            team_pairing.teamplayerpairing_set.filter(board_number=board_number)
            .exclude(white=None)
            .exclude(black=None)
            .nocache()
            .first()
    ...
    availability_url = abs_url(
        reverse("by_league:by_season:edit_availability", args=[league.tag, season.tag])
    )
    if player is not None:
        message_to_replaced_player = (
            "@%s: I am searching for an alternate to replace you for round %d, since you have been marked as unavailable. To stop the search and set yourself available again, <%s|click here>."
    ...
    if pairing is not None:
        ...
        if opponent.is_available_for(round_):
            message_to_opponent = (
                "@%s: Your opponent, @%s, has been marked as unavailable. I am searching for an alternate for you to play, please be patient."
    ...
    message = (
        '%sI have started searching for an alternate for <@%s> on board %d of "%s" in round %d.'
        % (
            _captains_ping(team, round_),
            _slack_user(player),
            board_number,
            team.name,
            round_.number,
...
def alternate_search_reminder(season, team, board_number, round_, **kwargs):
    ...
    message = (
        '%sI am still searching for an alternate for <@%s> on board %d of "%s" in round %d.'
        % (
            _captains_ping(team, round_),
            _slack_user(player),
            board_number,
...
@receiver(
    signals.alternate_search_all_contacted, dispatch_uid="heltour.tournament.notify"
)
def alternate_search_all_contacted(
    season, team, board_number, round_, number_contacted, **kwargs
):
    league = season.league
    # Broadcast a message to both team captains
    message = (
        '%sI have messaged every eligible alternate for board %d of "%s". Still waiting for responses from %d.'
...
def alternate_search_failed(season, team, board_number, round_, **kwargs):
    ...
    # Broadcast a message to both team captains
    message = (
        '%sSorry, I could not find an alternate for board %d of "%s" in round %d.'
...
def alternate_assigned(season, alt_assignment, **kwargs):
    ...
    if opponent is not None:
        opponent_notified = " Their opponent, @%s, has been notified." % _slack_user(
            opponent
        )
    else:
        opponent_notified = ""
    ...
    if aa.player == aa.replaced_player:
        message = (
            ...
            f"<@{_slack_user(aa.player)}> to play on board {aa.board_number} of "
            f'"{aa.team.name}" for round {aa.round_number}.{opponent_notified}'
        )
    else:
        message = (
            '%sI have assigned <@%s> to play on board %d of "%s" in place of <@%s> for round %d.%s'
            % (
                _captains_ping(aa.team, aa.round),
                _slack_user(aa.player),
                aa.board_number,
                aa.team.name,
                _slack_user(aa.replaced_player),
                aa.round.number,
                opponent_notified,
            )
        )
...
def _notify_alternate_and_opponent(league, aa):
    ...
    if team_pairing is None:
        ...
        message_to_alternate = (
            '@%s: You will be playing on board %d of "%s" for round %d.%s'
            % (
                _slack_user(aa.player),
                aa.board_number,
                aa.team.name,
    ...
    pairing = (
        team_pairing.teamplayerpairing_set.filter(board_number=aa.board_number)
        .exclude(white=None)
        .exclude(black=None)
        .nocache()
        .first()
    ...
    if not opponent.is_available_for(aa.round):
        ...
        message_to_alternate = (
            '@%s: You are playing on board %d of "%s".%s\n'
    ...
    if aa.player == aa.replaced_player:
        message_to_opponent = (
            "@%s: Your opponent, <@%s>, no longer requires an alternate. Please contact <@%s> as soon as possible."
            % (
                _slack_user(opponent),
                _slack_user(aa.replaced_player),
                _slack_user(aa.player),
        ...
        message_to_opponent = (
            "@%s: Your opponent, @%s, has been replaced by an alternate. Please contact your new opponent, <@%s>, as soon as possible."
            % (
                _slack_user(opponent),
                _slack_user(aa.replaced_player),
                _slack_user(aa.player),
        ...
        message_to_opponent = (
            "@%s: Your opponent has been replaced by an alternate. Please contact your new opponent, <@%s>, as soon as possible."
...
def alternate_needed(
    alternate, round_, response_time, accept_url, decline_url, **kwargs
):
    league = round_.season.league
    player = alternate.season_player.player
    setting = PlayerNotificationSetting.get_or_default(
        player=player, type="alternate_needed", league=league, offset=None
    )
    ...
    message = (
        "@%s: A team needs an alternate for %s. Would you like to play? Please click one of the following links within %s.\n<%s|Yes, I want to play>\n<%s|No, maybe next week>"
        % (
            _slack_user(alternate.season_player),
            round_str,
            _offset_str(response_time),
            abs_url(accept_url),
            abs_url(decline_url),
        )
    )
    ...
    if setting.enable_lichess_mail:
        ...
        li_msg = (
            "A team needs an alternate for %s. Please check Slack for more information.\n"
            % round_str
...
def alternate_spots_filled(alternate, response_time, **kwargs):
    ...
    if alternate.status == "unresponsive":
        message = (
            "All available alternate spots have now been filled. You've been moved to the bottom of the list since you didn't respond within %s."
            % _offset_str(response_time)
        )
    else:
...
def send_pairing_notification(
    type_, pairing, im_msg, mp_msg, li_subject, li_msg, offset=None, player=None
):
    ...
    white_setting = PlayerNotificationSetting.get_or_default(
        player=pairing.white, type=type_, league=league, offset=offset
    )
    black_setting = PlayerNotificationSetting.get_or_default(
        player=pairing.black, type=type_, league=league, offset=offset
    )
    use_mpim = (
        white_setting.enable_slack_mpim and black_setting.enable_slack_mpim and mp_msg
    )
    ...
    common_params = {
        ...
        "scheduling_channel": scheduling.slack_channel
        if scheduling is not None
        else "#scheduling",
        "scheduling_channel_link": scheduling.channel_link()
        if scheduling is not None
        else "#scheduling",
    }
    white_params = {
    ...
    if (
        send_to_white
        and (white_setting.enable_slack_im or white_setting.enable_slack_mpim)
        and not use_mpim
        and im_msg
    ):
    ...
    if (
        send_to_black
        and (black_setting.enable_slack_im or black_setting.enable_slack_mpim)
        and not use_mpim
        and im_msg
    ):
...
def notify_players_round_start(round_, **kwargs):
    ...
    if not round_.publish_pairings or round_.is_completed:
        logger.error(
            "Could not send round start notifications due to incorrect round state: %s"
            % round_
        )
        return
    unavailable_players = {
        pa.player
        for pa in PlayerAvailability.objects.filter(round=round_, is_available=False)
    ...
    with cache.lock("round_start"):
        for pairing in round_.pairings.select_related("white", "black"):
            if season.alternates_manager_enabled() and (
                pairing.white in unavailable_players
                or pairing.black in unavailable_players
            ):
...
def notify_players_game_time(pairing, **kwargs):
    ...
    if pairing.game_link == "" and pairing.result == "":
        send_pairing_notification(
            "game_time", pairing, im_msg, mp_msg, li_subject, li_msg
        )
...
def before_game_time(player, pairing, offset, **kwargs):
    im_msg = (
        "Reminder: Your game will start in {offset}.\n"
        + "<@{white}> (_white pieces_) vs <@{black}> (_black pieces_)"
    )
...
def notify_players_unscheduled(round_, **kwargs):
    im_msg = (
        "Reminder: Your game is currently unscheduled.\n"
        + "<@{white}> (_white pieces_) vs <@{black}> (_black pieces_)\n"
        + "When you have agreed on a time, post it in {scheduling_channel_link}.\n"
    ...
    if not round_.publish_pairings or round_.is_completed:
        logger.error(
            "Could not send unscheduled notifications due to incorrect round state: %s"
            % round_
        )
        return
    unavailable_players = {
    ...
    for pairing in round_.pairings.filter(
        result="", game_link="", scheduled_time=None
        ...
        if season.alternates_manager_enabled() and (
            ...
            continue
        send_pairing_notification(
            "unscheduled_game", pairing, im_msg, mp_msg, li_subject, li_msg
        )
        time.sleep(1)
...
def game_warning(pairing, warning, **kwargs):
    im_msg = (
        "Important: Your game is not valid because *%s*\n" % warning
    ...
    send_pairing_notification(
        "game_warning", pairing, im_msg, mp_msg, li_subject, li_msg
    )
...
def mod_request_created(instance, **kwargs):
    review_url = abs_url(
        reverse("admin:tournament_modrequest_review", args=[instance.pk])
    )
    message = "<@%s> created a request: <%s|%s>" % (
        _slack_user(instance.requester),
        review_url,
...
def notify_unresponsive(round_, player, punishment, allow_continue, pairing, **kwargs):
    ...
    message = (
        "Notice: You haven't messaged your %s opponent in the provided chat. "
        % league.name
        + "You are required to message your opponent within %s of the round start. "
        % _offset_str(league.get_leaguesetting().contact_period)
        + punishment
        + "\n"
        + "If you've messaged your opponent elsewhere, <%s|click here> to send a screenshot to the mods."
        % appeal_url
    )
    if allow_continue:
        continue_url = abs_url(
            reverse(
                "by_league:by_season:modrequest",
        ...
        message += (
            "\nIf you haven't but want to continue playing next round, <%s|click here>."
            % continue_url
        )
    _message_user(league, _slack_user(player), message)
    ...
    if league.competitor_type == "team":
        ...
        if tpp.white == player:
            ...
            team = tpp.black_team()
        message = (
            '%s<@%s> appears to be unresponsive on board %d of "%s" in round %d.'
            % (
                _captains_ping(team, round_),
                _slack_user(player),
                tpp.board_number,
...
@receiver(
    signals.notify_scheduling_draw_claim, dispatch_uid="heltour.tournament.notify"
)
def notify_scheduling_draw_claim(round_, player, **kwargs):
    ...
    message = (
        "Notice: Your %s game has been ruled a scheduling draw. " % league.name
...
@receiver(
    signals.notify_opponent_unresponsive, dispatch_uid="heltour.tournament.notify"
)
def notify_opponent_unresponsive(round_, player, opponent, pairing, **kwargs):
    ...
    if league.competitor_type != "team":
        message = (
            "Notice: Your %s opponent hasn't messaged you in the provided chat. "
            % league.name
...
def notify_noshow(round_, player, opponent, **kwargs):
    ...
    message = (
        "Notice: It appears your opponent, <@%s>, has not shown up for your scheduled game time in %s. "
...
def notify_noshow_claim(round_, player, punishment, allow_continue, **kwargs):
    ...
    appeal_url = abs_url(
        reverse(
            "by_league:by_season:modrequest",
            args=[league.tag, season.tag, "appeal_noshow"],
        )
    )
    message = (
        "Notice: You didn't show up for your scheduled game time in %s. " % league.name
    ...
    if allow_continue:
        ...
        message += (
            "\nOtherwise, if you want to continue playing next round, <%s|click here>."
            % continue_url
        )
    message += (
...
def notify_mods_unresponsive(round_, warnings, yellows, reds, **kwargs):
    ...
    message = (
        "The following actions have been taken for unresponsive players:"
...
def _captains_ping(team, round_):
    ...
    return (
        ""
        if len(captains) == 0
        else "<@%s>: " % _slack_user(captains[0])
        if len(captains) == 1
        else "<@%s>, <@%s>: " % (_slack_user(captains[0]), _slack_user(captains[1]))
    )
