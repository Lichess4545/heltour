...
@receiver(signals.notify_scheduling_draw, dispath_uid='heltour.tournament.notify')
def notify_scheduling_draw(round_,player,opponent, pairing, **kwargs):
    season = round_.season
    league = season.league
    appeal_url = abs_url(reverse('by_league:by_season:modrequest', args=[league.tag, season.tag, 'appeal_scheduling_draw']))
    message = 'Notice: Your %s opponent has claimed a scheduling draw. ' % league.name \
            + 'If you disagree with this, <%s|click here> to appeal and provide reasons and a screenshot of your conversation.' %appeal_url
    _message_user(league, _slack_user(player),message)
