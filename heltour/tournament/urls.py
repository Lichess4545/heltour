...
season_urlpatterns = [
    ...
    url(r'^registration_success/$', views.RegistrationSuccessView.as_view(),
        name='registration_success'),
    url(r'^rosters/$', views.RostersView.as_view(), name='rosters'),
    url(r'^standings/$', views.StandingsView.as_view(), name='standings'),
    url(r'^standings/section/(?P<section>[\w-]+)/$', views.StandingsView.as_view(),
    ...
    url(r'^pairings/team/(?P<team_number>[0-9]+)/$', views.PairingsView.as_view(),
        name='pairings_by_team'),
    url(r'^pairings/team/(?P<team_number>[0-9]+)/calendar.ics$', views.ICalPairingsView.as_view(),
        name='pairings_by_team_icalendar'),
    url(r'^round/(?P<round_number>[0-9]+)/pairings/$', views.PairingsView.as_view(),
        name='pairings_by_round'),
    url(r'^round/(?P<round_number>[0-9]+)/pairings/team/(?P<team_number>[0-9]+)/$',
        views.PairingsView.as_view(), name='pairings_by_round_team'),
    url(r'^stats/$', views.StatsView.as_view(), name='stats'),
    url(r'^dashboard/$', staff_member_required(views.LeagueDashboardView.as_view()),
        name='league_dashboard'),
    url(r'^player/$', views.UserDashboardView.as_view(), name='user_dashboard'),
    url(r'^player/(?P<username>[\w-]+)/$', views.PlayerProfileView.as_view(),
        name='player_profile'),
    ...
    url(r'^request/(?P<req_type>\w+)/success/$', views.ModRequestSuccessView.as_view(),
    ...
    url(r'^round/(?P<round_number>[0-9]+)/alternate/accept/$', views.AlternateAcceptView.as_view(),
        name='alternate_accept'),
    url(r'^round/(?P<round_number>[0-9]+)/alternate/decline/$',
