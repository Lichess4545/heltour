...
That within-run cache does not survive an ephemeral GitHub-hosted runner between separate workflow runs — the runner, and the Dagger engine container on it, are thrown away afterward. Cross-run persistence needs [Dagger Cloud](https://docs.dagger.io/): set a `DAGGER_CLOUD_TOKEN` repository secret and `docker-build.yml` picks it up automatically (passed as `env:` on the `build`/`publish` steps). **This secret is not currently set — an ops TODO, not a hard requirement.** Without it, every workflow run still gets full within-run caching (a `publish` step reuses everything `build` just did); it simply starts cold on the next run instead of resuming a warm cache. The old buildx-bake registry cache (`docker-bake.hcl`'s `cache_to`/`cache_from` writing `heltour-*:buildcache` tags to `ghcr.io`) is superseded by this and no longer runs from CI — `docker-bake.hcl` remains as the local-only build path for §2–5 above, untouched.
